...
with sync_playwright() as p:
    ...
    for language in ("en", "de", "fa"):
        for layout in layouts:
            for width in (320, 390, 768, 1024, 1440):
                ...
                check(page.locator(".reviews__counter").inner_text() == {"en": "1 of 1", "de": "1 von 1", "fa": "1 از 1"}[language], "Localized 1 of 1 counter")
                check(page.locator("[data-review-prev]").is_disabled() and page.locator("[data-review-next]").is_disabled(), "Single review disables navigation")
                ...
                check(page.locator(".review__link").get_attribute("href") == "https://booklife.com/project/ace-await-110051", "Verified public source URL")
    ...
    for language in ("en", "de", "fa"):
        for layout in layouts:
            ...
            visit(page, language, layout, True)
            check(page.locator("[data-review-id]").count() == 3, "Three fixture slides")
            ...
            check(page.locator("[data-review-id='preview-short']").is_visible(), "Next shows unscored review")
            ...
            check(page.locator("[data-review-id='preview-short']").is_visible(), "Real touch swipe navigates")
            client.detach()
    ...
    if args.website:
        ...
        check(page.locator(".reviews__counter").inner_text() == "1 of 1", "Fallback counter")
    ...
    check(static.locator("[data-review-id]:visible").count() == 3, "No-JS keeps all reviews visible")
    ...
    check(static.locator(".reviews__counter").inner_text() == "1 of 3", "No-JS fallback count matches articles")
    if args.website:
        static.goto(args.base + "/#reviews", wait_until="domcontentloaded")
        check(static.locator(".reviews__counter").inner_text() == "1 of 1", "No-JS website shows 1 of 1")
        check(static.locator(".review__link").is_visible(), "No-JS review remains readable")

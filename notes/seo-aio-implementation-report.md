...
| File | Change |
|---|---|
| `ace/index.html`, `ace-de/index.html`, `ace-fa/index.html` | New title/description/OG/Twitter metadata; canonical + hreflang to slash form; robots meta extended; JSON-LD graph rebuilt; creator attribution in first paragraph; "How to cite ACE" section and its table-of-contents entry (an "ACE at a glance" block was added and then removed at the author's request); language/brand links to slash form; 120 px logo; unused font removed; asset `?v=` bumped. FA footer author spelling corrected (عبدی‌جو → عابدی‌جو, matching the homepage). |
...
| `css/ace.css` | Styles for `.ace-cite` (+ responsive, RTL, print rules) and the definition-quote indent bar. The `.ace-glance` rules were added and then removed again on 13 September 2026 at the author's request. |
...
- ~~**ACE at a glance** (`#at-a-glance`)~~ — **removed by the author on 13 September 2026** in all three languages, together with its contents entry and its `.ace-glance` styles. The same facts remain machine-readable in the ACE model's JSON-LD (`alternateName`, `description`, `keywords`) and readable in the page text, so no structured-data claim lost its visible support.
...
- The cite block is in the table of contents and was checked visually at 1440 px and 390 px (EN, DE, FA).

**Author revision of the Agency definition (13 September 2026), EN/DE/FA.** The opening of the Agency section was rewritten: the agent is now "capable of making decisions and taking action"; the definition quote reads "Agency is the capacity of an agent to spend energy to make a proper decision and act—either maintaining its current state or moving into a new state."; *Conditions* is redefined as the relevant circumstances in which the agent exists and acts; and a new paragraph states that a decision does not require a predefined desired state. The `desired state` definition and its `#desired-state` anchor were therefore removed (nothing linked to it); `#current-state` and `#conditions` remain. The second quotation in that block ("'Agency' refers to any natural or legal agent that…") was also removed at the author's request, in all three languages; the same passage remains untouched inside the full ACE Manifesto further down each page, which is source text. The definition quotes in that block now carry a left indent bar (`.ace-definition blockquote`, mirrored to the right in Persian) so they read as quotations, matching the rest of the page.

## 5. Items not changed, and why

- **Homepage description** — kept; it already carries the brand and "decision model" and the prompt asks to preserve the literary identity. The homepage **title** was set by the author on 13 September 2026 to "ACE Conscious Studio | Decision Model & Stories", with the German "ACE Conscious Studio | Entscheidungsmodell & Geschichten" and the Persian "ACE Conscious Studio | مدل تصمیم‌گیری و داستان‌ها". The same string is used for `og:title`, `twitter:title`, the JSON-LD `WebPage.name` and the runtime `meta.title` / `meta.ogTitle` in `js/i18n.js`, so the static and runtime titles stay identical.
